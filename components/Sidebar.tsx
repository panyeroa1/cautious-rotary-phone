...
export default function Sidebar() {
  ...
  const selectedPersona = personaList.find(p => p.id === personaId) ?? personaList[0];
                ...
                >
                  {personaList.map(persona => (
                    <option
                      ...
                      value={persona.id}
                    >
                      {persona.displayName} ({persona.nationality})
                    </option>
                  ))}
                </select>
              </label>
              <p className="sidebar-persona-hint">
                Accent: {selectedPersona.accentDescription}
                <br />
                Expressions: {selectedPersona.expressionExamples.join(' · ')}
              </p>
            </fieldset>
            
            <h4 className="sidebar-section-title">Audio Settings</h4>
             <label>
                Output Volume: {Math.round(audioSettings.volume * 100)}%
                <input
                  type="range"
                  min="0"
                  max="2"
                  ...
                  onChange={e =>
                    setAudioSettings({ volume: parseFloat(e.target.value) })
                  }
                />
              </label>
              <label>
                Pitch (Detune): {audioSettings.pitch} cents
                <input
                  type="range"
                  min="-600"
                  max="600"
                  ...
                  onChange={e =>
                    setAudioSettings({ pitch: parseFloat(e.target.value) })
                  }
                />
              </label>
            <label>
              Playback Rate: {audioSettings.rate}x
                <input
                  type="range"
                  min="0.5"
                  max="2.0"
                  ...
                  onChange={e =>
                    setAudioSettings({ rate: parseFloat(e.target.value) })
                  }
                />
              </label>
          </div>
