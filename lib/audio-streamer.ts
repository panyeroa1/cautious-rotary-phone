...
export class AudioStreamer {
  ...
  public pitch: number = 0; // Detune in cents
  public rate: number = 1; // Playback rate
  ...
  private scheduleNextBuffer() {
    ...
    while (
      ...
      // Calculate effective duration based on playback rate and detune.
      // Detune in cents: 1200 cents = 1 octave = 2x frequency.
      // Effective speed multiplier = playbackRate * 2^(detune/1200)
      const effectiveRate = this.rate * Math.pow(2, this.pitch / 1200);
      const bufferDuration = audioBuffer.duration / effectiveRate;

      // Ensure we never schedule in the past
      const startTime = Math.max(this.scheduledTime, this.context.currentTime);
      source.start(startTime);
      this.scheduledTime = startTime + bufferDuration;
    }
